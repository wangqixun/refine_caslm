...
def loss_is_nan(losses):
    if isinstance(losses, torch.Tensor):
        if torch.isnan(losses).sum():
            return True
        else:
            return False
        ...
        for k in losses.keys():
            res.append(loss_is_nan(losses[k]))
        return sum(res) >= 1
    else:
        res = []
        for v in losses:
            res.append(loss_is_nan(v))
        return sum(res) >= 1
...
class TwoStageDetector(_TwoStageDetector):
    ...
    def forward_train(self,
        ...
        # losses_wo_nan = {}
        # for k in losses.keys():
        #     if not loss_is_nan(losses[k]):
        #         losses_wo_nan[k] = losses[k]
        # losses = losses_wo_nan
