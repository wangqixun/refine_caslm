...
class RSU5(nn.Module):#UNet05DRES(nn.Module):
    ...
    def forward(self,x):
        ...
        # hx = self.pool1(hx1)
        hx = torch.max_pool2d(hx1, kernel_size=[2,2])

        hx2 = self.rebnconv2(hx)
        # hx = self.pool2(hx2)
        hx = torch.max_pool2d(hx2, kernel_size=[2,2])

        hx3 = self.rebnconv3(hx)
        # hx = self.pool3(hx3)
        hx = torch.max_pool2d(hx3, kernel_size=[2,2])
...
class RSU3F(nn.Module):#UNet04FRES(nn.Module):

    def __init__(self, in_ch=3, mid_ch=12, out_ch=3):
        super(RSU3F,self).__init__()
...
class U2NETP(nn.Module):
    ...
    def __init__(self,in_ch=3,out_ch=1):
        ...
        self.stage1 = RSU5(in_ch,16,64)
        # self.pool12 = nn.MaxPool2d(2,stride=2,ceil_mode=True)

        self.stage2 = RSU4(64,16,64)
        # self.pool23 = nn.MaxPool2d(2,stride=2,ceil_mode=True)

        self.stage3 = RSU3(64,16,64)
        # self.pool34 = nn.MaxPool2d(2,stride=2,ceil_mode=True)

        self.stage4 = RSU3F(64,16,64)
    ...
    def forward(self,x):
        ...
        return d0, [d0, d1, d2, d3, d4]



@HEADS.register_module()
class U2MaskHead(BaseModule):
    ...
    def __init__(self,
        ...
        super(U2MaskHead, self).__init__(init_cfg)
    ...
    def init_weights(self):
        super(U2MaskHead, self).init_weights()
    ...
    def forward(self, x):
        ...
        return mask_pred, mask_pred_all
...
@HEADS.register_module()
class U2MaskSimpleLossHead(BaseModule):
    ...
    def __init__(self,
        ...
        assert init_cfg is None, 'To prevent abnormal initialization ' \
                                 'behavior, init_cfg is not allowed to be set'
        super(U2MaskSimpleLossHead, self).__init__(init_cfg)
        self.upsample_cfg = upsample_cfg.copy()
        if self.upsample_cfg['type'] not in [
                None, 'deconv', 'nearest', 'bilinear', 'carafe'
        ]:
