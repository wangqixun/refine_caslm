...
class CascadeLastMaskRefineRoIHead(BaseRoIHead, BBoxTestMixin, MaskTestMixin):
    ...
    def simple_test_mask_one_img(self, x, img_metas, det_bboxes, det_labels, index, rescale, glbctx_feat=None):
        ...
        if det_bboxes.shape[0] == 0:
            ...
            for i in range(0, det_labels.shape[0], interval):
                mask_results = self._mask_forward(x, mask_rois[i: i + interval], det_labels[i: i + interval], glbctx_feat=glbctx_feat)
    ...
    def simple_test_mask(self, x, img_metas, det_bboxes, det_labels, rescale=False, glbctx_feat=None):
        """Simple test for mask head without augmentation."""
        ...
        if all(det_bbox.shape[0] == 0 for det_bbox in det_bboxes):
            ...
            for index in range(num_imgs):
                segm_result = self.simple_test_mask_one_img(x, img_metas, det_bboxes, det_labels, index, rescale=rescale, glbctx_feat=glbctx_feat)
                segm_results.append(segm_result)
        return segm_results
    ...
    def simple_test(self, x, proposal_list, img_metas, rescale=False):
        ...
        rois = bbox2roi(proposal_list)
        for i in range(self.num_stages):
            bbox_results = self._bbox_forward(i, x, rois, glbctx_feat=glbctx_feat)
        ...
        if not self.with_mask:
            ...
            segm_results = self.simple_test_mask(
                x, img_metas, det_bboxes, det_labels, rescale=rescale, glbctx_feat=glbctx_feat)
