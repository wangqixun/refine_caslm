...
def imshow_det_bboxes(img,
    ...
    for i, (bbox, label) in enumerate(zip(bboxes, labels)):
        ...
        if segms is not None:
            ...
            elif segms.dtype == np.uint8:
                mask = (segms[i]>128).astype(bool)
            elif segms.dtype == np.float:
                mask = (segms[i]>0.5).astype(bool)
            ...
            if 0:  # 边缘
                ...
                for contour in contours:
                    cv2.drawContours(img_, [contour], -1, (182, 193, 255), thickness=2)
                img = img_
