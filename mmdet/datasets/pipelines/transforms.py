...
class Mixup(object):
    ...
    def get_img2(self):
        ...
        self.img2_fn = self.all_labels['images'][idx2]['file_name']
        img2_id = self.all_labels['images'][idx2]['id']
        img2_path = self.img_path + self.img2_fn
    ...
    def __call__(self, results):
        if self.mixup == True:
            if random.uniform(0, 1) > self.prob:
                ...
                h_ratio = img1.shape[0]/img2.shape[0]
                w_ratio = img1.shape[1]/img2.shape[1]
                resize_ratio = min(h_ratio, w_ratio)
                img2 = cv2.resize(img2, (int(img2.shape[1]*resize_ratio), int(img2.shape[0]*resize_ratio)))
                boxes2 = [[
                    np.float32(x1*resize_ratio), 
                    np.float32(y1*resize_ratio), 
                    np.float32(x2*resize_ratio), 
                    np.float32(y2*resize_ratio)] for x1, y1, x2, y2 in boxes2]
                if labels2 == []:
                ...
                # print(results['gt_labels'], self.img2_fn, results['img_info']['file_name'])
